...
all_valid_zones = set()
for i, (param1, op1, threshold1, param2, op2, threshold2, year) in enumerate(medium_3_test_cases):
    obj_path = f'/Users/mihokoda/Desktop/CityLLM/test_results/med/3/tc_med_3_{i}/objective.csv'
    obj = pd.read_csv(obj_path)
    obj_zones = set(obj['zone_id'].unique())    
    
    valid_zones_df = medium_3(param1, op1, threshold1, param2, op2, threshold2, year)
    result_zones = set(valid_zones_df['zone_id'].unique())
    
    all_valid_zones.update(result_zones)
    print(f"Test case {i} (params: {param1}, {op1}, {threshold1}, {param2}, {op2}, {threshold2}, {year}):")
