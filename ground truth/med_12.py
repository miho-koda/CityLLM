...
all_valid_zones = set()
for i, (neighbor_count, population_threshold, poi_threshold, logic, top_category, sub_category, pop_op, poi_op) in enumerate(medium_12_test_cases):
    obj_path = f'/Users/mihokoda/Desktop/CityLLM/test_results/med/12/tc_med_12_{i}/objective.csv'
    obj = pd.read_csv(obj_path)
    obj_zones = set(obj['zone_id'].unique())    
    
    valid_zones_df = medium_12(neighbor_count, population_threshold, poi_threshold, logic, top_category, sub_category, pop_op, poi_op)
    result_zones = set(valid_zones_df['zone_id'].unique())
    
    all_valid_zones.update(result_zones)
    print(f"Test case {i} (params: {neighbor_count}, {population_threshold}, {poi_threshold}, {logic}, {top_category}, {sub_category}, {pop_op}, {poi_op}):")
