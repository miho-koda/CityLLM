...
all_valid_zones = set() 
for i, (year, filters) in enumerate(hard_1_test_cases):
    obj_path = f'/Users/mihokoda/Desktop/CityLLM/test_results/hard/1/tc_hard_1_{i}/objective.csv'
    obj = pd.read_csv(obj_path)
    obj_zones = set(obj['zone_id'].unique())    
    
    kwargs = {}
    for param, op, threshold in filters:
        # Only allow ">=", "<=", ">", "<", "≤", "≥"
        # Encode operator into the value so `hard_1()` can check it
        if op in {">", "<", ">=", "<=", "≥", "≤"}:
            kwargs[param] = f"{op}{threshold}"
        else:
            raise ValueError(f"Unsupported operator: {op}")

    valid_zones_df = hard_1(year, **kwargs)

    result_zones = set(valid_zones_df['zone_id'].unique())
    
    all_valid_zones.update(result_zones)
    print(f"Test case {i} (params: {year}, {filters}):")
