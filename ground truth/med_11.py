...
all_valid_zones = set()
for i, (top_category, sub_category, competitor_threshold, comp_op, parking_threshold, parking_op, logic) in enumerate(medium_11_test_cases):
    obj_path = f'/Users/mihokoda/Desktop/CityLLM/test_results/med/11/tc_med_11_{i}/objective.csv'
    obj = pd.read_csv(obj_path)
    obj_zones = set(obj['zone_id'].unique())    
    
    valid_zones_df = medium_11(competitor_threshold, parking_threshold, top_category, sub_category, logic, comp_op, parking_op)
    result_zones = set(valid_zones_df['zone_id'].unique())
    
    all_valid_zones.update(result_zones)
    print(f"Test case {i} (params: {top_category}, {sub_category}, {competitor_threshold}, {comp_op}, {parking_threshold}, {parking_op}, {logic}):")
