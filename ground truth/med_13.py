...
all_valid_zones = set()
for i, (poi_num, max_distance, transport_type, top_categories, sub_categories, logic) in enumerate(medium_13_test_cases):
    obj_path = f'/Users/mihokoda/Desktop/CityLLM/test_results/med/13/tc_med_13_{i}/objective.csv'
    obj = pd.read_csv(obj_path)
    obj_zones = set(obj['zone_id'].unique())    
    
    valid_zones_df = medium_13(poi_num, max_distance, transport_type, top_categories, sub_categories, logic)
    result_zones = set(valid_zones_df['zone_id'].unique())
    
    all_valid_zones.update(result_zones)
    print(f"Test case {i} (params: {poi_num}, {max_distance}, {transport_type}, {top_categories}, {sub_categories}, {logic}):")
