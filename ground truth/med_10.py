...
all_valid_zones = set()
for i, (percent_pois_within_distance, distance_threshold_meters, transport_type, min_transport_count, logic, percent_op, count_op) in enumerate(medium_10_test_cases):
    obj_path = f'/Users/mihokoda/Desktop/CityLLM/test_results/med/10/tc_med_10_{i}/objective.csv'
    obj = pd.read_csv(obj_path)
    obj_zones = set(obj['zone_id'].unique())    
    
    valid_zones_df = medium_10(percent_pois_within_distance, distance_threshold_meters, transport_type, min_transport_count, logic, percent_op, count_op)
    result_zones = set(valid_zones_df['zone_id'].unique())
    
    all_valid_zones.update(result_zones)
    print(f"Test case {i} (params: {percent_pois_within_distance}, {distance_threshold_meters}, {transport_type}, {min_transport_count}, {logic}, {percent_op}, {count_op}):")
