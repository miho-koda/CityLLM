...
# Constants
PROJECT_ROOT = "/Users/mihokoda/Desktop/CityLLM"
LOGISTICS_PATH = os.path.join(PROJECT_ROOT, "test_results", "logistics.csv")
...
def run_single_prompt(prompt, base_dir, router_fn, llm_name, x):
    ##############################################
    # #3o tesing only
    # # Extract test case information from the directory path
    # test_case = os.path.basename(base_dir)
    # case_dir = base_dir
    ...
    py_path = os.path.join(case_dir, f"{llm_name}CoT.py")
    out_csv_path = os.path.join(case_dir, f"{llm_name}CoT.csv")
    ...
    cleaned_code_escaped = cleaned_code.replace('"""', '\"\"\"')
...
try:
    ...
    from code.site_selection.loader import get_poi_spend_dataset, get_parking_dataset
    from code.site_selection.zone import create_zone, assign_parking_zones, get_zone_center, get_neighbor_zones
    from code.site_selection.analysis import get_spendparam_years, get_num_parking, get_largest_parking_lot_area, get_largest_parking_capacity, get_distance_km
    from code.site_selection.filter import filter_df_based_on_zone, filter_pois_by_top_category, filter_pois_by_sub_category, get_transport_pois_in_zone
    from code.site_selection.population import get_population
    from code.prompt import dataframe_documentation, in_house_functions_documentation
...
def main():
    ...
    try:
        ...
        if isinstance(result, pd.DataFrame):
            ...
            pd.DataFrame([result]).to_csv("{out_csv_path}", index=False)
    except Exception:
...
def run_all_prompts(prompts, output_path, router_fn, llm_name):
    for x, prompt_text in enumerate(prompts):
        #print(prompt_text, output_path, router_fn, llm_name, x)
        run_single_prompt(prompt_text, output_path, router_fn, llm_name, x)
...
for router_function, model_id in routers_and_models:
    # # Simple prompts
    # for i in range(1, 19):            
    #         run_all_prompts(eval(f"simple_{i}"), f"/Users/mihokoda/Desktop/CityLLM/test_results/sim/{i}", router_function, model_id)
    
    # # Medium prompts
    # for i in range(1, 17):
    #     run_all_prompts(eval(f"medium_{i}"), f"/Users/mihokoda/Desktop/CityLLM/test_results/med/{i}", router_function, model_id)
